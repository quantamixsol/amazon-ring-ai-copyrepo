# main_app.py — unified config (single dropdown), excel preview, filenames shown,
# Fallbacks when only one file is uploaded (show both names with markers),
# Advanced settings placed ABOVE the copy-type selector.
...
# ---------- Header ----------
st.title("💡 Ring Copy Assistant")
st.caption("Create brand-compliant copy for Ring products — fast, friendly, and on-tone.")

# =========================================================
#              CONFIGURATION — Unified Control
# =========================================================
...
# Track *effective* sources actually used after fallbacks.
...
if use_custom_assets:
    ...
    # Excel (optional here — if missing, we'll fall back to default and show its name)
    ...
    # PDF (optional here — if missing, fall back to default and show its name)
    ...
    # If Excel wasn't uploaded or loaded, try default as fallback
    if excel_source is None:
        if not ss.get("file_loaded"):
            ...
            # Previously loaded (likely default from earlier run)
            excel_source = "uploaded" if ss.get("uploaded_name") and ss.get("uploaded_bytes") else "default"
            attached_excel_name = ss.get("uploaded_name") or os.path.basename(DEFAULT_EXCEL_PATH)

    # If PDF wasn't uploaded, fall back to default PDF
    ...
    # Defaults mode: load default Excel (if not already loaded)
...
# --- Status lines + Excel slim preview ---
...
# Show attached filenames clearly (mark default vs uploaded)
...
def run_generation(user_feedback: str = ""):
    ...
    # Determine PDF source robustly:
    # If an uploaded PDF exists, prefer it; else use the default PDF.
    pdf_bytes = None
    ...
    # Build content data (either a specific row or compact excerpt)
    if ss.use_specific_pui:
        selected_col = st.session_state.get("id_col")
